...
indice<-as.data.frame(read.table("/home/arw/estaciones_vielca.csv",sep = ";",header = FALSE))
...
id_estacion <- indice$V1[2:length(indice$V1)]
#nombre_estacion <-matrix(indice$V2[2:length(indice$V2)])
nombre_estacion <-indice$V2[2:length(indice$V2)]
...
###############################################################################################################################
###############################################################################################################################
#Funciones para extraer 1 datos.
#----------------------------------------------------------------------------------------------------------->
{
  #Funcion que extrae el último dato de una estación telemétrica
  revisa_estacion.1<-function(id_est,nombre,variable){
    set.seed(1000)
    n<-1
    k<-1
    dat<-rnorm(n*k)
    ...
    lista_coords <- data.frame(data=dat,data=dat)
    names(lista_coords)<-c("Lat","Lon")
    lista_fechas<-data.frame(data=dat,data=dat,data=dat,data=dat)
    names(lista_fechas)<-c("año","mes","dia","hora")
    
    consulta.ultimas.fechas.estacion<-paste("select horafecha as hora, medicion as temperatura from mediciones where estacionid = '",id_est,"' and parametroid ='",variable,"' order by hora desc limit 1",sep = "")
    #consulta.ultimas.fechas.estacion<-paste("select horafecha as hora, medicion as temperatura from mediciones where estacionid = '61' and parametroid ='AT' order by hora desc limit 1",sep = "")
    #consulta.ultimas.fechas.estacion<-"select horafecha as hora, medicion as temperatura from mediciones where estacionid = '61' and parametroid ='BP' order by hora desc limit 1"
    
    a<-dbGetQuery(con,consulta.ultimas.fechas.estacion)
    a
    #Extrae Latitud y Longitud
    lista_coords$Lat[1]<-indice$V3[[2]]
    ...
    x<-paste(lista_fechas$año[1],lista_fechas$mes[1],lista_fechas$dia[1],sep = "-")
    ...
    dato<-data.frame(nombre,x,lista_fechas$hora,lista_datos)
    names(dato)<-c("Estacion","fecha","Hora","Variable")
  ...
  extrae.1<-function(i,j,k){
    tryCatch(
      expr = {
        a<-revisa_estacion.1(i,j,k)
        a
      },
      error = function(e){
        a<-"Sin datos"
        #a<-data.frame("NIL","NIL","NIL","NIL")
        #names(a)<-c("Estacion","fecha","Hora","Variable")
        #a<-data.frame(j,"Sin datos","Sin datos","Sin datos","Sin datos")
        #names(a)<-names("Estacion","Año","Mes","Día","Hora","Variable")
        a
      },
      warning = function(w) {
...
###############################################################################################################################
###############################################################################################################################
#Funciones para extrae solo el ultimo dato sin header
#----------------------------------------------------------------------------------------------------------->
{
  #Funcion que extrae el último dato de una estación telemétrica
  revisa_dato_estacion.1<-function(id_est,nombre,variable){
    set.seed(1000)
    n<-1
    k<-1
    dat<-rnorm(n*k)
    ...
    dato<-data.frame(lista_datos$data)
    names(dato)<-c("Variable")
  ...
  #--------------------------------------------------------------------------------------------------
  #Ejecuta un Try & Catch para cuando la función no encuentre datos en alguna estación
  extrae.solo.dato.1<-function(i,j,k){
    tryCatch(
      expr = {
        a<-revisa_dato_estacion.1(i,j,k)
        a
      },
      error = function(e){
        a<-"No datos"
...
###############################################################################################################################
###############################################################################################################################
#Funciones para extraer ultimos  datos.
#----------------------------------------------------------------------------------------------------------->
{
  #Funcion que extrae el último dato de una estación telemétrica
  revisa_estacion.1d<-function(id_est,nombre,variable){
    set.seed(1000)
    n<-1
    k<-168
    dat<-rnorm(n*k)
    ...
    consulta.ultimas.fechas.estacion<-paste("select horafecha as hora, medicion as temperatura from mediciones where estacionid = '",id_est,"' and parametroid ='",variable,"' order by hora desc limit 168",sep = "")
    #consulta.ultimas.fechas.estacion<-"select horafecha as hora, medicion as temperatura from mediciones where estacionid = '61' and parametroid ='AT' order by hora desc limit 70"
    a<-dbGetQuery(con,consulta.ultimas.fechas.estacion)
    #Extrae fecha
    # lista_fechas$año[1]<-substr(a$hora[1],1,4)
    # lista_fechas$año[2]<-substr(a$hora[2],1,4)
    ...
    dato<-data.frame(a$hora,a$temperatura)
    names(dato)<-c("Fecha_Hora","Variable")
  ...
  #--------------------------------------------------------------------------------------------------
  #Ejecuta un Try & Catch para cuando la función no encuentre datos en alguna estación
  extrae.1d<-function(i,j,k){
    tryCatch(
      expr = {
        a<-revisa_estacion.1d(i,j,k)
        a
      },
      error = function(e){
        a<-"No datos"
