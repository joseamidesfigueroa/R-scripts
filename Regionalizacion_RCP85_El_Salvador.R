...
jpeg(filename = "C:/CursoManagua/resultados/esa_RCP85.jpg",width = 1800, height = 1500)
par(mfrow=c(2,2))
time_serie1<-ts(ProyTx_nicaragua_MPIESMMR_RCP85$proyeccion[[1]],start = c(2006,01,01),end = c(2100,12,31),frequency=12)
plot(time_serie1,main="Estacion 1",col="blue")

time_serie2<-ts(ProyTx_nicaragua_MPIESMMR_RCP85$proyeccion[[2]],start = c(2006,01,01),end = c(2100,12,31),frequency=12)
plot(time_serie1,main="Estacion 2",col="red")

time_serie3<-ts(ProyTx_nicaragua_MPIESMMR_RCP85$proyeccion[[3]],start = c(2006,01,01),end = c(2100,12,31),frequency=12)
plot(time_serie1,main="Estacion 3",col="black")

time_serie4<-ts(ProyTx_nicaragua_MPIESMMR_RCP85$proyeccion[[4]],start = c(2006,01,01),end = c(2100,12,31),frequency=12)
...
series_rcp85 <- as.data.frame(ProyTx_nicaragua_MPIESMMR_RCP85$proyeccion[])
colnames(series)=estaciones


jpeg("C:/CursoManagua/resultados/esa_prom_RCP85.jpg",width =1200, height = 800 )
promedio_nacional_rcp85<-as.data.frame(rowMeans(series_rcp85,dims = 1))
serie_temporal_promedio_nacional_rcp85 <- ts(promedio_nacional_rcp85,start = c(2006,01,01),end = c(2100,12,31),frequency=365)
plot(serie_temporal_promedio_nacional_rcp85, main="Serie de tiempo de la temperatura máxima diaria promedio nacional para El Salvador desde 2006 a 2100\nModelo MPIESMMR_RCP85", ylab="Temperaturas Celsius", xlab="Años",col="red",
     ylim=c(23,40))
dev.off()
